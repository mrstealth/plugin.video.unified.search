...
class Plugin():
    def __init__(self):
        self.id = 'plugin.video.test.plugin.b'
    ...
    def unified_search(self, keyword):
        ...
        video = {
            "url": "http://techslides.com/demos/sample-videos/small.flv",
            "image": "http://images.wikia.com/fileformats/images/a/ab/Icon_FLV.png",
            "title": "Sample video (%s) [FLV]" % self.addon.getAddonInfo('name'),
            "plugin": self.id
        }
    ...
    def play(self, url):
        self.log("xbmc.Player().play(%s)" % str(url))
